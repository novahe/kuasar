...
EOF
	...
	kubectl apply -f "${BATS_TMPDIR}/test-job.yaml"

	# Wait for job to complete
	kubectl wait --for=condition=complete --timeout=60s job/test-job

	# Check job status
	kubectl get job test-job -o yaml

	# Verify job completed successfully
	completed=$(kubectl get job test-job -o jsonpath='{.status.succeeded}')
	[ "$completed" = "1" ]

	# Get pod logs from the job
	pod_name=$(kubectl get pods -l job-name=test-job -o name | head -1 | sed 's/pod\///')
	if [ -n "$pod_name" ]; then
