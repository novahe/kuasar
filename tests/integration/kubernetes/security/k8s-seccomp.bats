...
EOF
	...
	kubectl apply -f "${BATS_TMPDIR}/seccomp-configmap.yaml" -n "${KUASAR_TEST_NAMESPACE}"
...
EOF
	...
	# Cleanup
	kubectl delete configmap seccomp-profile -n "${KUASAR_TEST_NAMESPACE}"
}
...
EOF
	...
	kubectl apply -f "${BATS_TMPDIR}/pod-privileged.yaml" -n "${KUASAR_TEST_NAMESPACE}"
	...
	pod_status=$(kubectl get pod privileged-pod -n "${KUASAR_TEST_NAMESPACE}" -o jsonpath='{.status.phase}' 2>/dev/null || echo "Pending")
	...
	if [ "$pod_status" = "Running" ]; then
		...
		kubectl describe pod privileged-pod -n "${KUASAR_TEST_NAMESPACE}"
	else
