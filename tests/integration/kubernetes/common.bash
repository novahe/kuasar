...
# Note: ERR trap disabled for bats compatibility
...
function setup_test_namespace() {
	info "Setting up test namespace: ${KUASAR_TEST_NAMESPACE}"
	if ! kubectl get namespace "${KUASAR_TEST_NAMESPACE}" &>/dev/null; then
		kubectl create namespace "${KUASAR_TEST_NAMESPACE}"
		info "Created namespace: ${KUASAR_TEST_NAMESPACE}"
	else
		info "Namespace ${KUASAR_TEST_NAMESPACE} already exists"
	fi
	export KUASAR_TEST_NAMESPACE
}

# Cleanup function for tests - only cleans resources in test namespace
function cleanup_test_resources() {
	info "Cleaning up test resources in namespace: ${KUASAR_TEST_NAMESPACE}..."
	kubectl delete pods --all -n "${KUASAR_TEST_NAMESPACE}" --ignore-not-found=true --timeout=30s 2>/dev/null || true
	kubectl delete deployments --all -n "${KUASAR_TEST_NAMESPACE}" --ignore-not-found=true --timeout=30s 2>/dev/null || true
	kubectl delete services --all -n "${KUASAR_TEST_NAMESPACE}" --ignore-not-found=true --timeout=30s 2>/dev/null || true
	kubectl delete configmaps --all -n "${KUASAR_TEST_NAMESPACE}" --ignore-not-found=true --timeout=30s 2>/dev/null || true
	kubectl delete secrets --all -n "${KUASAR_TEST_NAMESPACE}" --ignore-not-found=true --timeout=30s 2>/dev/null || true
	kubectl delete jobs --all -n "${KUASAR_TEST_NAMESPACE}" --ignore-not-found=true --timeout=30s 2>/dev/null || true
	kubectl delete cronjobs --all -n "${KUASAR_TEST_NAMESPACE}" --ignore-not-found=true --timeout=30s 2>/dev/null || true
	info "Cleanup completed for namespace: ${KUASAR_TEST_NAMESPACE}"
}

# Delete test namespace (optional, for complete cleanup)
function delete_test_namespace() {
	info "Deleting test namespace: ${KUASAR_TEST_NAMESPACE}"
	kubectl delete namespace "${KUASAR_TEST_NAMESPACE}" --ignore-not-found=true --timeout=60s 2>/dev/null || true
}
