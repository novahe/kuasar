...
EOF
	...
	resolv_conf=$(kubectl exec -n "${KUASAR_TEST_NAMESPACE}" "${pod_name}" -n "${KUASAR_TEST_NAMESPACE}" -- cat /etc/resolv.conf)
...
EOF
	...
	kubectl exec -n "${KUASAR_TEST_NAMESPACE}" "${pod_name}" -- nslookup kubernetes.default.svc.cluster.local || echo "Cluster DNS lookup test"
}
...
EOF
	...
	if kubectl exec -n "${KUASAR_TEST_NAMESPACE}" "${pod_name}" -n "${KUASAR_TEST_NAMESPACE}" -- which nslookup >/dev/null 2>&1; then
		info "nslookup available, testing DNS resolution..."
		# Cluster DNS should fail
		if kubectl exec -n "${KUASAR_TEST_NAMESPACE}" "${pod_name}" -n "${KUASAR_TEST_NAMESPACE}" -- nslookup kubernetes.default.svc.cluster.local 2>&1 | grep -q "NXDOMAIN"; then
...
EOF
	...
	kubectl apply -f "${BATS_TMPDIR}/pod-custom-dns.yaml" -n "${KUASAR_TEST_NAMESPACE}"
	sleep 5

	# Patch pod with custom DNS configuration
	kubectl patch -n "${KUASAR_TEST_NAMESPACE}" pod custom-dns-pod -p '{"spec":{"dnsPolicy":"None"}}' >/dev/null 2>&1 || true

	# Check pod status
	pod_phase=$(kubectl get pod -n "${KUASAR_TEST_NAMESPACE}" custom-dns-pod -o jsonpath='{.status.phase}')
	echo "Pod phase: $pod_phase"

	if [ "$pod_phase" = "Running" ]; then
		# Verify /etc/resolv.conf can be read
		kubectl exec -n "${KUASAR_TEST_NAMESPACE}" custom-dns-pod -- cat /etc/resolv.conf || true
	fi
}
...
EOF
	...
	kubectl apply -f "${BATS_TMPDIR}/pod-dns-config.yaml" -n "${KUASAR_TEST_NAMESPACE}"

	# Add DNS config via pod spec patch
	kubectl patch -n "${KUASAR_TEST_NAMESPACE}" pod dns-config-pod -p '{"spec":{"dnsPolicy":"None"}}' >/dev/null 2>&1 || true

	# Wait for pod to be ready
	k8s_wait_pod_be_ready "dns-config-pod" "default" "60"

	# Verify /etc/resolv.conf can be accessed
	kubectl exec -n "${KUASAR_TEST_NAMESPACE}" dns-config-pod -- cat /etc/resolv.conf || true
}
...
EOF
	...
	# Check DNS configuration
		kubectl exec -n "${KUASAR_TEST_NAMESPACE}" "${pod_name}" -- cat /etc/resolv.conf | head -5
		echo "---"

		# Cleanup for next iteration
		kubectl delete -n "${KUASAR_TEST_NAMESPACE}" --ignore-not-found=true pod "${pod_name}" --ignore-not-found=true
		sleep 3
	done
}
...
EOF
	...
	kubectl apply -f "${BATS_TMPDIR}/dns-test-svc.yaml" -n "${KUASAR_TEST_NAMESPACE}"
	k8s_wait_pod_be_ready "dns-test-backend" "${KUASAR_TEST_NAMESPACE}" "60"
...
EOF
	...
	kubectl apply -f "${BATS_TMPDIR}/dns-test-service.yaml" -n "${KUASAR_TEST_NAMESPACE}"
	sleep 5
...
EOF
	...
	kubectl apply -f "${BATS_TMPDIR}/dns-client-pod.yaml" -n "${KUASAR_TEST_NAMESPACE}"
	k8s_wait_pod_be_ready "dns-client" "${KUASAR_TEST_NAMESPACE}" "60"

	# Test DNS resolution for service
	if kubectl exec -n "${KUASAR_TEST_NAMESPACE}" dns-client -- which nslookup >/dev/null 2>&1; then
		# Try to resolve the service
		kubectl exec -n "${KUASAR_TEST_NAMESPACE}" dns-client -- nslookup dns-test-service.${KUASAR_TEST_NAMESPACE} || echo "DNS resolution completed"
		kubectl exec -n "${KUASAR_TEST_NAMESPACE}" dns-client -- wget -qO- --timeout=5 http://dns-test-service.${KUASAR_TEST_NAMESPACE} || echo "HTTP access completed"
	else
		info "nslookup not available, testing with wget"
		kubectl exec -n "${KUASAR_TEST_NAMESPACE}" dns-client -- wget -qO- --timeout=5 http://dns-test-service.${KUASAR_TEST_NAMESPACE} || echo "Test completed"
	fi

	# Cleanup
	kubectl delete -n "${KUASAR_TEST_NAMESPACE}" --ignore-not-found=true pod dns-client dns-test-backend --ignore-not-found=true
	kubectl delete -n "${KUASAR_TEST_NAMESPACE}" --ignore-not-found=true service dns-test-service --ignore-not-found=true
}
...
EOF
	...
	if kubectl get configmap -n "${KUASAR_TEST_NAMESPACE}" coredns -n kube-system >/dev/null 2>&1; then
...
EOF
	...
	kubectl exec -n "${KUASAR_TEST_NAMESPACE}" "${pod_name}" -- cat /etc/resolv.conf
}
...
EOF
	...
	kubectl exec -n "${KUASAR_TEST_NAMESPACE}" "${pod_name}" -- cat /etc/resolv.conf | grep "search"
}
...
teardown() {
	...
	kubectl delete -n "${KUASAR_TEST_NAMESPACE}" --ignore-not-found=true configmaps --all --ignore-not-found=true 2>/dev/null || true
