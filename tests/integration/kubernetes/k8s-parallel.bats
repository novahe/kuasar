...
EOF
		...
		kubectl apply -f "${BATS_TMPDIR}/parallel-pod-${i}.yaml"
	done

	# Wait for all pods to be ready
	for pod_name in "${pod_names[@]}"; do
		k8s_wait_pod_be_ready "${pod_name}" "default" "120"
	done

	# Verify all pods are running
	running_count=$(kubectl get pods -l app=parallel-test -o jsonpath='{.items[?(@.status.phase=="Running")].metadata.name}' | wc -w)
	[ "$running_count" -ge "$pod_count" ]

	# Test exec on all pods
	for pod_name in "${pod_names[@]}"; do
		pod_exec "${pod_name}" echo "Test from ${pod_name}"
	done
}
