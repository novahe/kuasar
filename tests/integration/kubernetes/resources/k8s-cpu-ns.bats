...
EOF
	...
	kubectl get pod "${pod_name}" -n "${KUASAR_TEST_NAMESPACE}" -o jsonpath='{.spec.containers[0].resources.limits.cpu}'
	[ "$?" = "0" ]
}
...
EOF
	...
	# Verify resource limits are set
	kubectl get pod "${pod_name}" -n "${KUASAR_TEST_NAMESPACE}" -o jsonpath='{.spec.containers[0].resources.limits.memory}'
	[ "$?" = "0" ]
}
