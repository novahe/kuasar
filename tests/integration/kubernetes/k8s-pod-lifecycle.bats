...
EOF
	...
	local count=0
	while kubectl get pod "${pod_name}" >/dev/null 2>&1 && [ $count -lt 30 ]; do
		sleep 1
		count=$((count + 1))
	done
...
EOF
	...
	local count=0
	while kubectl get pod "${pod_name}" >/dev/null 2>&1 && [ $count -lt 20 ]; do
		sleep 1
		count=$((count + 1))
	done
...
spec:
  ...
  - name: main-container
    image: ${BUSYBOX_IMAGE}
    command: ["sh", "-c", "echo 'Web server started' && nc -l -p 8080 -e echo 'HTTP OK' & sleep 3600"]
    lifecycle:
      postStart:
        httpGet:
          host: localhost
...
EOF
	...
	pod_name=$(k8s_create_pod "${BATS_TMPDIR}/pod-http-poststart.yaml}"

	# Wait for container to be ready and postStart to execute
	sleep 10

	# Verify pod is running
	is_pod_running "${pod_name}"

	# Check logs for any HTTP errors (httpGet to localhost should work or fail gracefully)
...
EOF
	...
	local max_wait=35
	local elapsed=0
	while kubectl get pod "${pod_name}" >/dev/null 2>&1 && [ $elapsed -lt $max_wait ]; do
		sleep 1
		elapsed=$((elapsed + 1))
	done

	info "Pod terminated in ${elapsed} seconds (within 30s grace period)"
	[ $elapsed -le $max_wait ]
}
...
EOF
	...
	kubectl apply -f "${BATS_TMPDIR}/pod-restart-never.yaml"

	# Wait for pod to complete
	sleep 5

	# Verify pod did not restart
	restart_count=$(kubectl get pod restart-never-pod -o jsonpath='{.status.containerStatuses[0].restartCount}')
	[ "$restart_count" = "0" ]

	# Verify pod phase is Failed
	phase=$(kubectl get pod restart-never-pod -o jsonpath='{.status.phase}')
	[ "$phase" = "Failed" ]
}
...
EOF
	...
	kubectl apply -f "${BATS_TMPDIR}/pod-restart-onfailure.yaml"

	# Wait for pod to start and possibly restart
	sleep 10

	# Check if pod restarted after initial failure
	phase=$(kubectl get pod restart-onfailure-pod -o jsonpath='{.status.phase}')
	...
	kubectl delete pod restart-onfailure-pod
}
