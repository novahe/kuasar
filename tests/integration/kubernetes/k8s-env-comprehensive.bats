...
EOF
	...
	kubectl apply -f "${BATS_TMPDIR}/test-configmap-env.yaml"
...
EOF
	...
	kubectl apply -f "${BATS_TMPDIR}/test-secret-env.yaml"
...
EOF
	...
	grep_pod_exec_output "${pod_name}" "POD_NAMESPACE=default" sh -c "echo POD_NAMESPACE=\$POD_NAMESPACE"
	...
	output=$(kubectl exec "${pod_name}" -- sh -c "echo POD_IP=\$POD_IP")
...
EOF
	...
	kubectl apply -f "${BATS_TMPDIR}/test-configmap-mixed.yaml"
...
EOF
	...
	kubectl apply -f "${BATS_TMPDIR}/test-secret-mixed.yaml"
...
EOF
	...
	kubectl delete secret mixed-secret
}
...
EOF
	...
	kubectl apply -f "${BATS_TMPDIR}/test-configmap-envfrom.yaml"
...
EOF
	...
	# Cleanup
	kubectl delete configmap envfrom-config
}
...
EOF
	...
	kubectl apply -f "${BATS_TMPDIR}/test-secret-envfrom.yaml"
...
EOF
	...
	# Cleanup
	kubectl delete secret envfrom-secret
}

teardown() {
	k8s_delete_all_pods || true
	kubectl delete configmaps --all --ignore-not-found=true 2>/dev/null || true
	kubectl delete secrets --all --ignore-not-found=true 2>/dev/null || true
