    ...
    F: FnOnce() + Send + 'static,
{
    ...
    tokio::spawn(async move {
        ...
        if mode == CopyMode::Console {
            ...
            copy_until_eof_or_exit(src, dst, exit_signal, close_signal, on_close).await;
        }
...
macro_rules! idle_timeout {
    ($fut:expr, $exited:expr, $exit_signal:expr, $drain_timeout:expr, $op_name:expr) => {{
        let mut pinned_fut = std::pin::pin!($fut);
        loop {
            if !$exited {
                tokio::select! {
                    res = &mut pinned_fut => break Ok(res),
                    _ = $exit_signal.wait() => {
                        $exited = true;
                        debug!("container exited, switching to drain mode with idle timeout {:?}", $drain_timeout);
                    }
                }
            } else {
                match tokio::time::timeout($drain_timeout, &mut pinned_fut).await {
                    Ok(res) => break Ok(res),
                    Err(_) => {
                        debug!("stdio drain {} timed out (idle) after {:?}", $op_name, $drain_timeout);
                        break Err(std::io::Error::new(ErrorKind::TimedOut, "timed out"));
                    }
                }
            }
        }
    ...
    F: FnOnce() + Send + 'static,
{
    ...
    loop {
        // Only read phase has an idle timeout to prevent hanging on guest failure.
        let read_res = idle_timeout!(
            src.read(&mut buffer),
            exited,
            exit_signal,
            drain_timeout,
        ...
        let n = match read_res {
            ...
            Ok(Err(e)) => {
                log_copy_result(Err(e));
                break;
            }
            Err(_) => break, // Timeout already logged
        };

        // Write phase MUST NOT timeout, as the host client (e.g. kubectl cp) might be slow.
...
mod tests {
    ...
    #[tokio::test]
    async fn test_output_drain_times_out_pending_read_after_exit() {
        ...
        // Signal exit immediately so the read timeout starts.
        exit_signal.signal();

        // The copy task should exit on its own because of the read timeout.
